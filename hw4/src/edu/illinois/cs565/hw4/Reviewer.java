...
/**
 * The Reviewer sample application will retrieve the completed assignments for a
 * given HIT, output the results and approve the assignment.
 * 
 * mturk.properties must be found in the current file path. You will need to
 * have the HIT ID of an existing HIT that has been accepted, completed and
 * submitted by a worker. You will need to have the .success file generated from
 * bulk loading several HITs (i.e. Site Category sample application).
 * 
 * The following concepts are covered: - Retrieve results for a HIT - Output
 * results for several HITs to a file - Approve assignments
 */
public class Reviewer {
	 ...
	 * 
	 * @param successFile
	 *            The .success file containing the HIT ID and HIT Type ID
	 * @param outputFile
	 *            The output file to write the submitted results to
	 */
	public void printResults(String successFile, String outputFile) {

		try {

			// Loads the .success file containing the HIT IDs and HIT Type IDs
			// of HITs to be retrieved.
			HITDataInput success = new HITDataCSVReader(successFile);

			// Retrieves the submitted results of the specified HITs from
			// Mechanical Turk
			...
			// The output file is a tab delimited file containing all relevant
			// details
			// of the HIT and assignments. The submitted results are included as
			// the last set of fields
			...
			System.err.println("ERROR: Could not print results: "
	...
	public void reviewAnswers(String hitId) {
		...
		for (Assignment assignment : assignments) {
			...
			// Only assignments that have been submitted will contain answer
			// data
			if (assignment.getAssignmentStatus() == AssignmentStatus.Submitted) {
				...
				// Calling a convenience method that will parse the answer XML
				// and extract out the question/answer pairs.
